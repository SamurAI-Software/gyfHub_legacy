...
func testUserFavoriteToOneFavoriteUsingFavorite(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadFavorite(ctx, tx, false, (*[]*UserFavorite)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserFavoriteToOneSetOpFavoriteUsingFavorite(t *testing.T) {
	var err error
...
var (
	userFavoriteDBTypes = map[string]string{`ID`: `text`, `UserID`: `text`, `FavoriteID`: `text`}
	_                   = bytes.MinRead
)
